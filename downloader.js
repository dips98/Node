  ...
  for (const resource of data.roomResourceList) {
    for (const asset of resource.roomAssetList) {
      ...
      try {
        ...
        const expectedSize = asset.totalBytes; // Assuming `totalBytes` property is present in the asset object
        if (downloadedSize === expectedSize) {
          console.log(`Successfully Downloaded: ${fileName}`);
        } else {
          console.log(`Partial download: ${fileName}. Expected size: ${expectedSize} bytes, Downloaded size: ${downloadedSize} bytes`);
        }
      } catch (err) {
